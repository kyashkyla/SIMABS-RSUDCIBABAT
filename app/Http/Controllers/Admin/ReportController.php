...
class ReportController extends Controller
{
     ...
     * Untuk export .xlsx / PDF asli, tambahkan paket
     * "maatwebsite/excel" dan "barryvdh/laravel-dompdf".
     */
     ...
     * Jalankan terlebih dahulu: composer require barryvdh/laravel-dompdf
     */
    public function exportPdf(Request $request)
    {
        ...
        $records = $attendances->map(fn (Attendance $a) => [
            'date' => $a->date->translatedFormat('d M Y'),
            'nip' => $a->employee->nip ?? '-',
            'name' => $a->employee->name ?? '-',
            'department' => $a->employee->department ?? '-',
            'checkIn' => $a->check_in ? $a->check_in->format('H:i') : '-',
